...
class PurchaseOrderPage extends StatefulWidget {
  ...
  @override
  _PurchaseOrderPageScreenState createState() =>
      _PurchaseOrderPageScreenState();
}

class _PurchaseOrderPageScreenState extends State<PurchaseOrderPage> {
  ...
  bool _shouldFetchNewCatalogs = false;
  ...
  final Map<String, TextEditingController> _controllers = {}; // qty controllers
  final Map<String, TextEditingController> _rateControllers =
      {}; // size purchase rate controllers
  final Map<String, TextEditingController> _shadeRateControllers =
      {}; // shade purchase rate controllers
          ...
          if (items is List && items.isNotEmpty) {
            ...
            EditOrderData.data =
                groupedByStyle.entries.map((entry) {
                  return _convertToCatalogOrderData(entry.key, entry.value);
                }).toList();
  ...
  void dispose() {
    ...
    _rateControllers.forEach((_, ctrl) => ctrl.dispose());
    _shadeRateControllers.forEach((_, ctrl) => ctrl.dispose());
    super.dispose();
  }
  ...
  Future<void> _initializeData() async {
    ...
    if (widget.docId == -2) {
      EditOrderData.partyList = _dropdownData.partyList;
      EditOrderData.brokerList = _dropdownData.brokerList;
      EditOrderData.transporterList = _dropdownData.transporterList;
    }
    if (widget.docId == -1) {
      if (EditOrderData.partyList.isNotEmpty)
        _dropdownData.partyList = EditOrderData.partyList;
      if (EditOrderData.brokerList.isNotEmpty)
        _dropdownData.brokerList = EditOrderData.brokerList;
      if (EditOrderData.transporterList.isNotEmpty)
        ...
        if (response.statusCode == 200) {
          ...
          for (var shade in selectedColors2[styleKey]!) {
            ...
            for (var size in orderMatrix.sizes) {
              ...
              _rateControllers[rateCtrlKey]!.addListener(() {
                final value =
  ...
  Future<Map<String, dynamic>> _fetchEditHeader(int docId) async {
    final response = await http.post(
      Uri.parse('${AppConstants.BASE_URL}/Purchase/headerDetailsForEdit'),
      headers: {'Content-Type': 'application/json'},
  ...
  Future<List<dynamic>> _fetchEditItems(int docId) async {
    final response = await http.post(
      Uri.parse('${AppConstants.BASE_URL}/Purchase/getPurchaseDataEdit'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({"doc_id": docId}),
    );
    if (response.statusCode == 200) {
  ...
  void _setDefaultDates() {
    ...
    _orderControllers.date.text = _PurchaseListControllers.formatDate(today);
    _orderControllers.deliveryDate.text = _PurchaseListControllers.formatDate(
      today,
    );
  ...
  void _restoreFormFromEditOrderData() {
    ...
    _orderControllers.deliveryDate.text =
        EditOrderData.deliveryDate.isNotEmpty
            ...
            : _PurchaseListControllers.formatDate(DateTime.now());
    ...
    for (var entry in styleMap.entries) {
      ...
      for (var shade in shades) {
        ...
        for (var size in sizes) {
          ...
          // Quantity controller
          final qtyKey = '$styleKey-$shade-$size';
          _controllers[qtyKey] = TextEditingController(text: qty.toString());

          // ---- PURCHASE RATE RESTORATION ----
          final purRate =
              double.tryParse(
                item['purRate']?.toString() ?? item['wsp']?.toString() ?? '0',
              ) ??
              0.0;
          ...
          final rateCtrlKey = 'PR-$styleKey-$shade-$size';
          _rateControllers[rateCtrlKey] = TextEditingController(
            text: purRate.toString(),
          );
        }
      }
    }
  ...
  void _copyToAllSizes(String styleKey, String shade, List<String> sizes) {
    ...
    _showSnackBar('Copied qty & rate to all sizes');
  }
    ...
    for (var order in catalogOrderList) {
      ...
      for (var shade in selectedColors2[styleKey]!) {
        ...
        for (var size in order.orderMatrix.sizes) {
          ...
          // 🔥 CRITICAL FIX: LISTENER TO UPDATE MAP
          _rateControllers[rateCtrlKey]!.addListener(() {
            final value =
                double.tryParse(_rateControllers[rateCtrlKey]!.text) ?? 0.0;
        ...
        final shadeRateKey = 'SR-$styleKey-$shade';
        _shadeRateControllers[shadeRateKey] = TextEditingController(text: '0');
      }
    }
  }
  ...
  void _restoreQuantitiesFromEditOrderData() {
    if (EditOrderData.detailsForEdit.isNotEmpty) {
      try {
        ...
        if (stored['rates'] != null) {
          ...
          for (var styleKey in rateJson.keys) {
            ...
            for (var shade in styleData.keys) {
              final shadeData = styleData[shade] as Map<String, dynamic>;
              shadeRates[styleKey]![shade] =
                  (shadeData['shadeRate'] as num?)?.toDouble() ?? 0.0;

              // Shade controller
              final shadeRateKey = 'SR-$styleKey-$shade';
              _shadeRateControllers.putIfAbsent(
                shadeRateKey,
                () => TextEditingController(
                  text: shadeRates[styleKey]![shade].toString(),
                ),
              );
              ...
              if (shadeData['sizes'] != null) {
                ...
                for (var size in sizesData.keys) {
                  ...
                  _rateControllers[rateCtrlKey]?.text = rate.toString();
                }
              }
            }
          }
        ...
        print('Error restoring quantities/rates: $e');
      }
    }
  }
  ...
  // ==================== SAVE Purchase (unchanged except using new rate map) ====================
  Future<void> _savePurchaseList() async {
    if (_isSaving) return;
    ...
    try {
      ...
      // 🔹 KEEP YOUR EXISTING data2 (NO CHANGE)
      final Map<String, dynamic> data2 = {
        "Purchasedate": formatDate(_orderControllers.date.text, true),
      ...
      for (var item in _selectedItems) {
        final String styleCode = item['styleCode']?.toString() ?? '';

        // ✅ FIX: map styleCode → styleKey
        final String styleKey =
            catalogOrderList
                .firstWhere(
                  (c) => c.catalog.styleCode == styleCode,
                  orElse: () => catalogOrderList.first,
                )
                .catalog
                .styleKey;

        final String shadeName = item['shadeName']?.toString() ?? '';
      ...
      final response = await http.post(
        Uri.parse(
          int.parse(EditOrderData.doc_id) > 0
              ? '${AppConstants.BASE_URL}/purchase/updatePurchaseOrder'
              : '${AppConstants.BASE_URL}/purchase/savePurchaseOrder',
        ),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(payload),
      );
  ...
  Future<void> _selectDate(TextEditingController controller) async {
    ...
    if (picked != null) {
      controller.text = _PurchaseListControllers.formatDate(picked);
  ...
  AppBar _buildAppBar() {
    return AppBar(
      ...
      title: Text(
        _isEditMode ? 'Edit Purchase Order' : 'Add Purchase Order',
        style: const TextStyle(
          color: Colors.white,
                  ...
                  _buildResponsiveRow(
                    ...
                    _buildTextField(
                      ...
                      onTap: () async {
                        ...
                        if (picked != null) {
                          _orderControllers.deliveryDate.text =
                              _PurchaseListControllers.formatDate(picked);
                          _orderControllers.deliveryDays.text =
    ...
    return Card(
      ...
      child: Column(
        children: [
          ...
          // ---- Shade data row (editable rate + copy icon) ----
          // ---- Shade data row (editable rate + copy icon) ----
          Container(
            color: Colors.white,
            child: Row(
              children: [
                // Shade name + copy quantities icon
                Expanded(
                  flex: 2,
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
  ...
  // Helper: cell with plain text
  Widget _buildCell(String text, int flex, {Color? color}) => Expanded(
    flex: flex,
    child: Container(
      ...
      child: SizedBox(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: ElevatedButton.icon(
                onPressed: _isSaving ? null : _savePurchaseList,
                icon:
                    _isSaving
                        ? const SizedBox(
                          width: 18,
  ...
  void _openCatalogSelection() async {
      ...
      MaterialPageRoute(
        builder:
            (context) => OrderBookingScreen(
              editMode: true,
              editModeType: Constants.PURCHASE_ORDER,
            ),
      ),
    );
    if (result != null && result is List<Catalog>) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => PurchaseOrderPage(docId: -1, catalogs: result),
        ),
      );
    }
  }

  // ========== FORM WIDGETS (unchanged) ==========
...
// ================== CONTROLLER CLASSES ==================
class _PurchaseListControllers {
...
class _PurchaseListDropdownData {
  ...
  Future<List<Map<String, String>>> _fetchLedgers(String ledCat) async {
    try {
      ...
      if (response['statusCode'] == 200 && response['result'] != null) {
        ...
        final List<KeyName> keyNames =
            rawList
                .map((json) => KeyName.fromJson(json as Map<String, dynamic>))
                .toList();
        return keyNames
