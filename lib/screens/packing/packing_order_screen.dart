...
class _PackingListScreenState extends State<PackingListScreen> {
  ...
  Future<void> _initializeData() async {
    ...
    _orderControllers.deliveryDate.text = _PackingListControllers.formatDate(today);
  ...
  Future<void> _loadBookingTypes() async {
    try {
      ...
      setState(() {
        _bookingTypes = (rawData as List).map((json) => Item(
          itemKey: json['key'], itemName: json['name'], itemSubGrpKey: '',
        )).toList();
      });
    } catch (e) { print('Failed to load booking types: $e'); }
  }
  ...
  void _openOrderListPage() async {
    if (_orderControllers.selectedPartyKey?.isEmpty ?? true) {
      _showValidationDialog('Party Selection Required', 'Please select a party before viewing Sales Orders.');
      return;
    }
  ...
  String getTodayWithZeroTime() {
    ...
    return DateFormat('yyyy-MM-dd HH:mm:ss.SSS').format(DateTime(now.year, now.month, now.day));
  }
    ...
    try {
      ...
      if (selectedConsigneeName != null && selectedConsigneeName.isNotEmpty) {
        final selectedConsignee = consignees.firstWhere(
          ...
          orElse: () => Consignee(ledKey: '', ledName: '', stnKey: '', stnName: '', paymentTermsKey: '', paymentTermsName: '', pytTermDiscdays: '0'),
        );
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Packing List saved successfully')));
      Navigator.of(context).pop();
    } catch (e) {
      print('Error: $e');
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error saving packing: $e')));
    } finally {
  ...
  void _handlePartySelection(String? val, String? key) async {
    ...
    try {
      ...
      setState(() {
        _orderControllers.updateFromPartyDetails(details, _dropdownData.brokerList, _dropdownData.transporterList);
  ...
  Future<void> _showAddMoreInfoDialog() async {
    if (UserSession.userType == 'S' && (_orderControllers.selectedPartyKey?.isEmpty ?? true)) {
      _showValidationDialog('Party Selection Required', 'Please select a party before adding more information.');
      return;
    }
    ...
    if (result != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Additional information saved'), backgroundColor: Colors.green));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Packing List', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 20)),
      ),
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    _buildTextField("Packing Date", _orderControllers.date, isDate: true, onTap: () => _selectDate(_orderControllers.date), isRequired: true),
                    _buildPartyDropdownRow(),
                    _buildDropdown("Broker", "B", _orderControllers.selectedBroker, (val, key) async {
                    ...
                    if (UserSession.userType == 'A') _buildTextField("Comm (%)", _orderControllers.comm),
                    _buildDropdown("Transporter", "T", _orderControllers.selectedTransporter, (val, key) => _orderControllers.selectedTransporterKey = key),
                    _buildResponsiveRow(
                      _buildTextField("Delivery Days", _orderControllers.deliveryDays, readOnly: true),
                      _buildTextField("Delivery Date", _orderControllers.deliveryDate, isDate: true, onTap: () async {
                        final today = DateTime.now();
                        final picked = await showDatePicker(context: context, initialDate: today, firstDate: today, lastDate: DateTime(2100));
                        if (picked != null) {
                    ...
                    // Order Type Radio - Fixed Alignment
                    Container(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: _buildRadioOption(
                                  ...
                                  _selectedSOOption,
                                  (value) {
                                    setState(() => _selectedSOOption = value);
                                  },
                                ),
                              ),
                            ],
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text('₹${_calculateGrandTotal().toStringAsFixed(2)}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: AppColors.primaryColor)),
                          ],
                        ),
                      ),
                    ],
  ...
  Widget _buildRadioOption(String value, String title, IconData icon, String? selectedValue, Function(String?) onChanged) {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Icon(icon, size: 20, color: isSelected ? AppColors.primaryColor : Colors.grey.shade600),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(color: AppColors.primaryColor.withOpacity(0.1), borderRadius: const BorderRadius.only(topLeft: Radius.circular(12), topRight: Radius.circular(12))),
            child: Row(
              children: [
                const Icon(Icons.shopping_cart, size: 18, color: AppColors.primaryColor),
                const SizedBox(width: 8),
                Text('Selected Items (${_selectedSOItems.length})', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.primaryColor)),
                const Spacer(),
                TextButton(onPressed: () => setState(() => _selectedSOItems.clear()), child: const Text('Clear All', style: TextStyle(color: Colors.red, fontSize: 12))),
              ],
            ),
          ),
          ListView.builder(
  ...
  Widget _buildSelectedItemCard(Map<String, dynamic> item, int index) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 2, offset: const Offset(0, 1))]),
      child: ExpansionTile(
        tilePadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
        childrenPadding: EdgeInsets.zero,
        leading: Container(width: 40, height: 40, decoration: BoxDecoration(color: AppColors.primaryColor.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.inventory, color: AppColors.primaryColor, size: 20)),
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(item['docNo'] ?? 'N/A', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF2C3E50))),
            const SizedBox(height: 2),
            Text(item['itemName'] ?? 'N/A', style: TextStyle(fontSize: 12, color: Colors.grey.shade700)),
          ],
        ),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.green.withOpacity(0.1), borderRadius: BorderRadius.circular(12)), child: Text('Qty: ${item['selectedQty']} ${item['unitName']}', style: const TextStyle(fontSize: 11, color: Colors.green, fontWeight: FontWeight.w500))),
            const SizedBox(width: 8),
            IconButton(icon: const Icon(Icons.delete_outline, color: Colors.red, size: 20), onPressed: () => setState(() => _selectedSOItems.removeAt(index)), padding: EdgeInsets.zero, constraints: const BoxConstraints()),
          ],
        ),
        children: [
          Container(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(child: _buildReadOnlyField('Product', item['itemName'] ?? 'N/A')),
                    const SizedBox(width: 8),
                    Expanded(child: _buildReadOnlyField('Design', item['styleCode'] ?? 'N/A')),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: _buildReadOnlyField('Type', item['typeName'] ?? 'N/A')),
                    const SizedBox(width: 8),
                    Expanded(child: _buildReadOnlyField('Shade', item['shadeName'] ?? 'N/A')),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: _buildReadOnlyField('Brand', item['brandName']?.isNotEmpty == true ? item['brandName'] : 'N/A')),
                    const SizedBox(width: 8),
                    Expanded(child: _buildReadOnlyField('Order No', item['docNo'] ?? 'N/A')),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: _buildReadOnlyField('Date', item['docDt'] ?? 'N/A')),
                    const SizedBox(width: 8),
                    Expanded(child: _buildReadOnlyField('Dlv Date', item['dlvDate'] ?? 'N/A')),
                  ],
                ),
                const SizedBox(height: 12),
                const Divider(),
                ...
                Row(
                  children: [
                    Expanded(child: _buildEditableField('MRP', item['mrp']?.toString() ?? '0', (val) => setState(() => item['mrp'] = double.tryParse(val) ?? 0))),
                    const SizedBox(width: 8),
                    Expanded(child: _buildEditableField('Rate', item['rate'].toString(), (val) => setState(() => item['rate'] = double.tryParse(val) ?? 0))),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: _buildEditableField('Qty', item['selectedQty'].toString(), (val) {
                      final double qty = double.tryParse(val) ?? 0;
                      setState(() {
                        item['selectedQty'] = qty.clamp(0.0, item['balQty']);
                        item['itemAmt'] = item['selectedQty'] * item['rate'];
                      });
                    })),
                    const SizedBox(width: 8),
                    Expanded(child: _buildReadOnlyField('Avg Rt', '${(item['rate']).toStringAsFixed(2)}')),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: _buildReadOnlyField('Item Amt', '₹${(item['selectedQty'] * item['rate']).toStringAsFixed(2)}')),
                    const SizedBox(width: 8),
                    Expanded(child: _buildReadOnlyField('Ord Qty', '${item['balQty']} ${item['unitName']}')),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: _buildEditableField('Disc (%)', item['discPercent']?.toString() ?? '0', (val) {
                      final double disc = double.tryParse(val) ?? 0;
                      setState(() {
                        item['discPercent'] = disc;
                        item['discAmt'] = (item['selectedQty'] * item['rate'] * disc) / 100;
                      });
                    })),
                    const SizedBox(width: 8),
                    Expanded(child: _buildEditableField('Disc Amt', item['discAmt']?.toString() ?? '0', (val) {
                      final double discAmt = double.tryParse(val) ?? 0;
                      setState(() {
                        item['discAmt'] = discAmt;
                        if ((item['selectedQty'] * item['rate']) > 0) {
                          item['discPercent'] = (discAmt * 100) / (item['selectedQty'] * item['rate']);
                        }
                      });
                    })),
                  ],
                ...
                Row(
                  children: [
                    Expanded(child: _buildReadOnlyField('Amount', '₹${((item['selectedQty'] * item['rate']) - (item['discAmt'] ?? 0)).toStringAsFixed(2)}')),
                    const SizedBox(width: 8),
                    Expanded(child: _buildEditableField('Amount Remark', item['amtRemark'] ?? '', (val) => setState(() => item['amtRemark'] = val))),
                  ],
                ),
                const SizedBox(height: 12),
                const Divider(),
                ...
                Align(
                  ...
                  child: Container(
                    ...
                    child: Text('Total Qty: ${item['selectedQty']} ${item['unitName']}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: AppColors.primaryColor)),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildSizeWiseTable(Map<String, dynamic> item) {
    final List<Map<String, dynamic>> sizes = item['sizes'] ?? [
      {'size': 'S', 'qty': 0, 'ordQty': 10, 'stock': 50, 'rate': item['rate'], 'mrp': item['mrp'], 'netRate': item['rate']},
      {'size': 'M', 'qty': 0, 'ordQty': 15, 'stock': 45, 'rate': item['rate'], 'mrp': item['mrp'], 'netRate': item['rate']},
      {'size': 'L', 'qty': 0, 'ordQty': 12, 'stock': 30, 'rate': item['rate'], 'mrp': item['mrp'], 'netRate': item['rate']},
      {'size': 'XL', 'qty': 0, 'ordQty': 8, 'stock': 20, 'rate': item['rate'], 'mrp': item['mrp'], 'netRate': item['rate']},
    ];
          ...
          child: Container(
            ...
            child: DataTable(
              ...
              columns: const [
                ...
                DataColumn(label: Text('Ord Qty', style: TextStyle(fontWeight: FontWeight.bold))),
                DataColumn(label: Text('Stock', style: TextStyle(fontWeight: FontWeight.bold))),
                DataColumn(label: Text('Rate', style: TextStyle(fontWeight: FontWeight.bold))),
                DataColumn(label: Text('MRP', style: TextStyle(fontWeight: FontWeight.bold))),
                DataColumn(label: Text('Net Rate', style: TextStyle(fontWeight: FontWeight.bold))),
              ],
              rows: sizes.map<DataRow>((size) {
                return DataRow(cells: [
                  DataCell(Text(size['size'])),
                  DataCell(SizedBox(
                    width: 60,
                    child: TextFormField(
                      initialValue: size['qty'].toString(),
                      keyboardType: TextInputType.number,
                  ...
                  DataCell(Text(size['ordQty'].toString())),
                  DataCell(Text(size['stock'].toString())),
                  DataCell(Text(size['rate'].toString())),
                  DataCell(Text(size['mrp'].toString())),
                  DataCell(Text(size['netRate'].toString())),
                ]);
              }).toList(),
            ),
          ),
            ...
            children: [
              Expanded(
                child: ElevatedButton(
                  ...
                  child: const Text("Add More Info", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                ),
              ),
              const SizedBox(width: 16),
              Expanded(
        ...
        child: Row(
          ...
          children: [
            _buildFABButton(Icons.add, 'Add Item', () {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Add Item feature coming soon')));
            }),
            Container(width: 1, height: 35, color: Colors.grey.shade300),
            _buildFABButton(Icons.qr_code_scanner, 'Barcode', () {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Barcode scanning feature coming soon')));
            }),
          ],
        ),
      ),
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              const SizedBox(width: 8),
              Text(label, style: TextStyle(color: AppColors.primaryColor, fontWeight: FontWeight.w600, fontSize: 13)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildPartyDropdownRow() => Padding(
    ...
    child: _buildDropdown("Party Name", "w", _orderControllers.selectedParty, _handlePartySelection, isEnabled: UserSession.userType != 'C', isRequired: true),
  );

  Widget _buildDropdown(String label, String ledCat, String? selectedValue, Function(String?, String?) onChanged, {bool isEnabled = true, bool isRequired = false}) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      child: DropdownSearch<String>(
        ...
        filterFn: (item, filter) => filter.isEmpty ? true : item.split('-->').first.trim().toLowerCase().contains(filter.toLowerCase()),
        ...
        dropdownDecoratorProps: DropDownDecoratorProps(
          dropdownSearchDecoration: InputDecoration(
            ...
            enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.grey.shade400), borderRadius: BorderRadius.circular(6)), 
            focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: const Color(0xFF2196F3), width: 2), borderRadius: BorderRadius.circular(6)), 
            labelStyle: const TextStyle(color: Color(0xFF64748B), fontSize: 13, fontWeight: FontWeight.w500)
          ),
        ),
        dropdownBuilder: (context, selectedItem) => Text(selectedItem ?? '', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 14)),
  ...
  List<Map<String, String>> _getLedgerList(String ledCat) {
    switch (ledCat) {
      case 'w': return _dropdownData.partyList;
      case 'B': return _dropdownData.brokerList;
      case 'T': return _dropdownData.transporterList;
      default: return [];
    }
  }

  String? _getKeyFromValue(String ledCat, String? value) => _getLedgerList(ledCat).firstWhere((e) => e['ledName'] == value, orElse: () => {'ledKey': ''})['ledKey'];
  ...
  Widget _buildTextField(String label, TextEditingController controller, {bool isDate = false, bool readOnly = false, VoidCallback? onTap, bool isText = false, bool isRequired = false}) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: const Color(0xFFCBD5E1)), borderRadius: BorderRadius.circular(6)),
          focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: AppColors.primaryColor, width: 1), borderRadius: BorderRadius.circular(6)),
          labelStyle: const TextStyle(color: Color(0xFF64748B), fontSize: 13, fontWeight: FontWeight.w500),
        ),
      ),
    );
...
class _PackingListControllers {
  ...
  void updateFromPartyDetails(Map<String, dynamic> details, List<Map<String, String>> brokers, List<Map<String, String>> transporters) {
    ...
    if (partyBrokerKey.isNotEmpty) {
      final broker = brokers.firstWhere((e) => e['ledKey'] == partyBrokerKey, orElse: () => {'ledName': ''});
    ...
    if (partyTrspKey.isNotEmpty) {
      final transporter = transporters.firstWhere((e) => e['ledKey'] == partyTrspKey, orElse: () => {'ledName': ''});
...
class _PackingListDropdownData {
  ...
  Future<void> loadAllDropdownData() async {
    try {
      final results = await Future.wait([_fetchLedgers("w"), _fetchLedgers("B"), _fetchLedgers("T"), _fetchLedgers("S")]);
      partyList = results[0];
      brokerList = results[1];
      transporterList = results[2];
      salesPersonList = results[3];
    } catch (e) { print('Error loading dropdown data: $e'); }
  }

  Future<Map<String, dynamic>> fetchLedgerDetails(String ledKey) async {
    final response = await http.post(Uri.parse('${AppConstants.BASE_URL}/users/getLedgerDetails'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({"ledKey": ledKey}));
    if (response.statusCode == 200) return jsonDecode(response.body);
    throw Exception('Failed to load details');
  }

  Future<List<Map<String, String>>> _fetchLedgers(String ledCat) async {
    final response = await http.post(Uri.parse('${AppConstants.BASE_URL}/users/getLedger'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({"ledCat": ledCat, "coBrId": UserSession.coBrId ?? '', "ledKey": UserSession.userLedKey ?? '', "userType": UserSession.userType ?? ''}));
    if (response.statusCode == 200) return (jsonDecode(response.body) as List).map((e) => {'ledKey': e['ledKey'].toString(), 'ledName': e['ledName'].toString()}).toList();
    throw Exception("Failed to load ledgers");
  }

  Future<String> fetchCommissionPercentage(String ledKey) async {
    final response = await http.post(Uri.parse('${AppConstants.BASE_URL}/users/getCommPerc'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({"ledKey": ledKey}));
