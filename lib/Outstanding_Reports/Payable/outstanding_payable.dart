...
import 'outstanding_payable_detail_page.dart';
...
class _OutstandingPayablePageState extends State<OutstandingPayablePage>
  ...
  void openDateFilterSheet() {
    showModalBottomSheet(
      context: context,
      builder: (context) {
        return DateFilterSheetForPayable(
  ...
  String _getPdfSubtitle(
    ...
    int ledgerCount = _getLedgerCount();
    if (ledgerCount > 0) {
      parts.add('$ledgerCount Ledger(s)');
    }
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final data = await OutstandingService.getOutstandingPayableBills();
      setState(() {
        _billsData = data;
        ...
        return InkWell(
          onTap: () {
            Navigator.push(
              ...
              MaterialPageRoute(
                builder:
                    (context) => OutstandingPayableDetailPage(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              const SizedBox(width: 2),
                              Text(
                                "Payable: ${(item['creditLimit'] ?? 0).toStringAsFixed(2)}",
                                style: GoogleFonts.poppins(
                                  fontSize: 11,
                                  color: Colors.green,
                                  fontWeight: FontWeight.w500,
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 2),
                            Text(
                              "Payable: ${creditLimit.toStringAsFixed(2)}",
                              style: GoogleFonts.poppins(
                                fontSize: 11,
                                color: Colors.green,
                                fontWeight: FontWeight.w500,
  ...
  void _showGraphBottomSheet(BuildContext context) {
    ...
    // Ageing Analysis for Payable (same as before)
    DateTime now = DateTime.now();
    DateTime today = DateTime(now.year, now.month, now.day);
...
// Date Filter Sheet for Payable
class DateFilterSheetForPayable extends StatelessWidget {
  final DateTime currentFromDate;
  final DateTime currentToDate;
  final Function(DateTime, DateTime) onApply;

  DateFilterSheetForPayable({
    super.key,
