...
class PackingOrderReportViewPage extends StatefulWidget {
  final String orderId; // This is the packing ID (e.g., "71")
  final String orderNo; // Packing number
  final String? defaultWhatsAppMobileNo;
  final bool fromRegisterPage;

  const PackingOrderReportViewPage({
    Key? key,
  ...
  @override
  _PackingOrderReportViewPageState createState() => _PackingOrderReportViewPageState();
}

class _PackingOrderReportViewPageState extends State<PackingOrderReportViewPage> {
  bool isLoading = true;
  bool pdfError = false;
  ...
  Future<void> _fetchReportData() async {
    try {
      final data = await ApiService.fetchPackingReportData(widget.orderId);
      setState(() {
        reportData = data;
  ...
  Future<void> _loadImages() async {
    for (var item in items) {
      ...
      for (var style in styles) {
        ...
        // Also load shade images
        List shades = style['shades'] ?? [];
        for (var shade in shades) {
          String shadeImageUrl = shade['shade_Img'] ?? '';
  ...
  Future<void> _sharePDF() async {
    try {
      ...
      final file = File('${tempDir.path}/Packing_${widget.orderNo}.pdf');
      await file.writeAsBytes(pdfBytes);
      await Share.shareXFiles([XFile(file.path)], text: 'Packing Order Report');
    } catch (e) {
              ...
              children: [
                ...
                pw.Expanded(
                  child: pw.Column(
                    children: [
                      pw.Text(
                        headerData['Co_Name']?.toString() ?? "VRS Enterprises",
                        style: pw.TextStyle(
                          fontSize: 18,
                  ...
                  children: [
                    pw.Row(
                      children: [
                        ...
                        pw.Text(
                          headerData['Doc_No']?.toString() ?? widget.orderNo,
                          style: pw.TextStyle(
                            fontSize: 12,
                            fontWeight: pw.FontWeight.bold,
                          ),
 ...
 pw.Widget _buildPDFItemsTable() {
  ...
  for (var category in items) {
    ...
    for (var style in styles) {
      ...
      if (styleImageUrl.isNotEmpty && imageCache.containsKey(styleImageUrl)) {
        ...
        print('Style image found for: $styleCode'); // Debug
      } else {
        print('Style image NOT found for: $styleCode, URL: $styleImageUrl'); // Debug
      }
      ...
      for (var shade in shades) {
        ...
        // Get shade-specific image from cache
        pw.ImageProvider? shadeImage;
        if (shadeImageUrl.isNotEmpty && imageCache.containsKey(shadeImageUrl)) {
          shadeImage = imageCache[shadeImageUrl];
          print('Shade image found for: $shadeName'); // Debug
        } else {
          print('Shade image NOT found for: $shadeName, URL: $shadeImageUrl'); // Debug
        }
        ...
        for (var size in sizeData) {
          ...
          if (sizeName.isNotEmpty) {
            // Accumulate quantities for same size
        ...
        List<pw.Widget> dataCells = [
          ...
          pw.Container(
            ...
            child: (shadeCount == 1 && styleImage != null)
        ...
        // Shade Image column - show shade image for EACH shade row
        dataCells.add(
          pw.Container(
            width: 35,
            height: 35,
    ...
    if (categoryStyleCount > 0) {
      ...
      int totalColumns = 4 + categorySizes.length + 2; // Changed from 3 to 4 (added Style column)
      ...
      tables.add(
        pw.Column(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
            pw.Text(categoryName, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
            ...
            pw.Container(
              ...
              child: pw.Table(
                ...
                columnWidths: {
                  0: const pw.FixedColumnWidth(45), // Style
                  1: const pw.FixedColumnWidth(50), // Style Image
                  2: const pw.FixedColumnWidth(50), // Shade
                  ...
                  (5 + categorySizes.length): const pw.FixedColumnWidth(50), // TotQty
                },
  ...
  int totalColumns = 4 + (items.isNotEmpty ? _getSizesForCategory(items.last).length : 0) + 2;
  ...
  for (int i = 4; i < totalColumns - 1; i++) {
    grandTotalRowCells.add(pw.Container(padding: const pw.EdgeInsets.all(4), child: pw.Container()));
  }

  grandTotalRowCells.add(
    pw.Container(
      padding: const pw.EdgeInsets.all(4),
      child: pw.Text("Grand Total: $grandTotalQty", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 8), textAlign: pw.TextAlign.center),
    ),
  );
  ...
  String _prepareOrderReportCaption() {
    ...
    return '''
*📦 PACKING ORDER REPORT*
━━━━━━━━━━━━━━━━━━━━

🏢 *Company:* ${headerData['Co_Name']?.toString() ?? 'VRS Enterprises'}
📦 *Packing No:* $orderNo
📅 *Date:* $orderDate
👤 *Party:* $partyName
...
*Generated from VRS ERP App*
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Preparing packing report for WhatsApp...',
  ...
  Future<void> _shareViaWhatsAppNode(String mobileNo) async {
    try {
      ...
      final response = await http.post(
        ...
        body: {
          'data': fileBase64,
          'filename': 'Packing_${widget.orderNo}.pdf',
          'key': whatsappKey,
          'number': '91$mobileNo',
          'caption': caption,
        },
      ...
      if (response.statusCode == 200 && mounted) {
        try {
            ...
            SnackBar(
              content: Text(
                responseData['status'] == 'success' 
                  ? '✓ Packing report sent successfully to $mobileNo'
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('✓ Packing report sent successfully to $mobileNo'),
  ...
  Future<void> _shareViaWhatsAppBackend(String mobileNo) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('📤 Sending packing report...'),
          duration: Duration(seconds: 2),
        ),
      );
      ...
      if (response.statusCode == 200 && mounted) {
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(responseBody['message'] ?? '✓ Packing report sent successfully'),
              backgroundColor: Colors.green,
              duration: const Duration(seconds: 3),
            ),
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('✓ Packing report sent successfully'),
              backgroundColor: Colors.green,
              duration: const Duration(seconds: 3),
            ),
          );
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Packing report will be sent as PDF via WhatsApp',
                        style: TextStyle(
                          fontSize: 12,
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: isLoading
            ...
            : pdfError
            ? const Center(child: Text("Error loading packing report"))
            : PdfPreview(
                build: (format) => _generatePDF(),
                allowSharing: false,
                allowPrinting: false,
                pdfFileName: 'Packing_${widget.orderNo}.pdf',
