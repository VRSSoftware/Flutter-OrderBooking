...
//         setState(() {
//           // Store the packing document number
//           _packingDocNo = response['docNo']?.toString() ?? '';
...
//               if (item['SalesOrderNo'] == null ||
//                   item['SalesOrderNo'].toString().isEmpty) {
//                 item['SalesOrderNo'] = item['SalesOrderNo'] ?? 'N/A';
//               }
...
//           children: [
//             Text(
//               displayDocNo, // This will show "DP00076"
//               style: const TextStyle(
//                 fontSize: 14,
//                 fontWeight: FontWeight.bold,
...
//                             'N/A', // Use SalesOrderNo from the item
//                       ),
//                     ),
//                   ],
//                 ),
...
class _PackingListAgainstSOState extends State<PackingListAgainstSO> {
  // ==================== VARIABLES ====================
  bool _isUpdateMode = false;
  String _packingDocNo = ''; // Added for storing packing document number
          ...
          for (var size in sizes) {
            ...
            if (qty > 0) {
              ...
              dataArray.add({
                ...
                "WSP": (size['rate'] as double? ?? 0).toInt().toString(),
                "size": size['size']?.toString() ?? '',
                "TotQty": ((item['selectedQty'] as double? ?? 0).toInt())
                    .toString(),
                "Note": item['amtRemark']?.toString() ?? '',
                "color": item['shadeName']?.toString() ?? '',
                "Qty": qty.toString(),
                ...
                children: [
                  ...
                  _buildDropdown(
                    ...
                    (val, key) async {
                      ...
                      if (key != null) {
                        final commission =
                            await _dropdownData.fetchCommissionPercentage(key);
                        _orderControllers.comm.text = commission;
                      }
                    },
                    isEnabled: UserSession.userType != 'C',
                  ...
                  Container(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      style: ElevatedButton.styleFrom(
                        backgroundColor: isPartySelected
                            ? AppColors.primaryColor
                            : Colors.grey.shade300,
                        foregroundColor: Colors.white,
                        padding: const EdgeInsets.symmetric(vertical: 12),
  ...
  Widget _buildSelectedItemCard(Map<String, dynamic> item, int index) {
    ...
    if (totalQty != (item['selectedQty'] as double? ?? 0)) {
      item['selectedQty'] = totalQty.toDouble();
      item['itemAmt'] = (item['selectedQty'] as double? ?? 0) *
          (item['rate'] as double? ?? 0);
    }
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: _buildReadOnlyField(
                        'Order No',
                        item['SalesOrderNo']?.toString() ?? 'N/A',
                      ),
                    ),
                  ],
                ),
