...
//                   Icon(Icons.arrow_back_ios, color: AppColors.primaryColor, size: 16),
//                 Text(
...
class _EditOrderScreenBarcodeState extends State<EditOrderScreenBarcode> {
  ...
  Future<void> _initializeData() async {
    ...
    try {
      ...
      if (docId == '-1') {
        response = http.Response(jsonEncode(EditOrderData.detailsForEdit), 200);
      }
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _orderControllers.selectedBrokerKey = data['Broker_Key']?.toString();
          _orderControllers.selectedBroker = broker['ledName'] ?? '';
          _orderControllers.comm.text =
              data['Broker_Comm']?.toString() ?? '0.00';
          ...
          _orderControllers.selectedTransporterKey =
              data['Trsp_Key']?.toString();
          _orderControllers.selectedTransporter = transporter['ledName'] ?? '';

          _orderControllers.deliveryDays.text =
              data['dlv_Days']?.toString() ?? '0';
          _orderControllers.deliveryDate.text =
              data['DlvDate'] != null
                  ? _OrderControllers.formatDate(DateTime.parse(data['DlvDate']))
                  : _OrderControllers.formatDate(DateTime.now());
          _orderControllers.remark.text = data['Remark']?.toString() ?? '';
        });
      } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to fetch order details: ${response.statusCode}'),
          ),
        );
      }
    } catch (e) {
    ...
    await Future.wait([
      _styleManager.fetchOrderItems(doc_Id: docId.toString()),
    ]);
                      ...
                      return sizes.asMap().entries.map((sizeEntry) {
                        ...
                        return {
                          ...
                          'wsp':
                              ...
                              0.0,
                          'barcode': catalogOrder.catalog.barcode,
                          'note': "",
                        };
                      }).toList();
                    })
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: _activeTab == ActiveTab.transaction
...
class EditStyleCard extends StatelessWidget {
  final String styleCode;
  final CatalogOrderData catalogOrder;
  final Map<String, Map<String, int>> quantities;
  final Set<String> selectedColors;
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      _buildInfoRow('Remark', catalog.remark),
                      _buildInfoRow(
                        'Stk Type',
                        catalog.upcoming_Stk == '1' ? 'Upcoming' : 'Ready',
                      ),
                      ...
                      _buildInfoRow(
                        'Order Amount',
                        '₹${_calculateCatalogPrice(catalogOrder).toStringAsFixed(2)}',
                        valueColor: Colors.purple[800],
                      ),
                    ],
                  ),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            color: getColor(shade).withOpacity(0.1),
            child: Text(
              shade,
              style: GoogleFonts.roboto(
                fontWeight: FontWeight.bold,
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                _buildHeaderCell("Size", width: 60),
                _buildHeaderCell("Qty", width: 70),
                _buildHeaderCell("MRP", width: 60),
                _buildHeaderCell("WSP", width: 60),
                _buildHeaderCell("Stock", width: 60),
              ],
            ),
          ),
          for (var size in sizes) ...[
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          ...
          _buildCell(rate, width: 60),
          _buildCell(wsp, width: 60),
          _buildCell(stock, width: 60),
        ],
      ),
    );
  }
    ...
    return Column(
      children: [
        ...
        buildTextField(
          context,
          "Party Name",
          TextEditingController(text: widget.controllers.selectedParty ?? ''),
          readOnly: true,
        ),
        const SizedBox(height: 8),
        _buildDropdown(
  ...
  Widget _buildDropdown(
    ...
    return Padding(
      ...
      child: DropdownSearch<String>(
        key: ValueKey('$ledCat-$selectedValue'),
        validator: (value) {
          if (label == "Broker" && (value == null || value.isEmpty)) {
...
Widget buildFullField(
  ...
  return Padding(
    ...
    child: buildTextField(context, label, controller, isText: true),
  );
}
