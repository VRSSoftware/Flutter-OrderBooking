...
import 'package:vrs_erp/PurchaseInward/PurchaseInwardAgainstSo/PurchaseInwardDetailsPage.dart';
import 'package:vrs_erp/PurchaseInward/PurchaseInwardAgainstSo/PurchaseInwardList.dart';
import 'package:vrs_erp/constants/app_constants.dart';
import 'package:vrs_erp/services/Purchase_services.dart';
import 'package:vrs_erp/services/app_services.dart';
import 'package:vrs_erp/models/keyName.dart';
import 'package:vrs_erp/services/production_services.dart';

class PurchaseInwardAgainstSO extends StatefulWidget {
  final String? inwardId;
  final Map<String, dynamic>? inwardData;

  const PurchaseInwardAgainstSO({Key? key, this.inwardId, this.inwardData})
      : super(key: key);

  @override
  _PurchaseInwardAgainstSOState createState() => _PurchaseInwardAgainstSOState();
}

class _PurchaseInwardAgainstSOState extends State<PurchaseInwardAgainstSO> {
  // ==================== VARIABLES ====================
  ...
  final TextEditingController _supRefNoCtrl = TextEditingController(text: '');
  ...
  // List to store added items from SO
  List<Map<String, dynamic>> addedItems = [];
  List<Map<String, dynamic>> selectedSOItems = [];
  ...
  // Store selected SO document IDs
  ...
  Future<void> _loadSeries() async {
    final seriesData = await ProductionService.getSeries('1'); // Purchase Inward series
  ...
  Future<void> _loadDocNo() async {
    final docNoData = await PurchaseService.getPurInwardDocNo();
    setState(() {
  ...
  Future<void> _loadInwardData(String inwardId) async {
    // Implement load existing inward data if needed
    try {
      // Similar to purchase return but for purchase inward
    } catch (e) {
  ...
  void _calculateTotals() {
    ...
    for (var item in addedItems) {
      ...
      if (sizes.isNotEmpty) {
        ...
        double rate = item['PurRate'] as double? ?? 0;
  ...
  void _openSOSelection() async {
    ...
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => SOForPurchaseInwardListScreen(
          supplierKey: selectedSupplierKey!,
          existingSelectedItems: selectedSOItems,
        ),
      ),
    );
    ...
    if (result != null && result is List && result.isNotEmpty) {
      setState(() {
        selectedSOItems.addAll(result.cast<Map<String, dynamic>>());
        addedItems.addAll(result.cast<Map<String, dynamic>>());

        _selectedDocDtlIds = selectedSOItems
  ...
  void _openDetailsDialog() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PurchaseInwardDetailsPage(
          supplierKey: selectedSupplierKey,
          supplierName: selectedSupplierName,
          supplierStation: selectedStationName,
          selectedItems: selectedSOItems,
          initialData: _detailsData,
        ),
      ),
    );
  ...
  Future<void> _saveInward() async {
    ...
    if (addedItems.isEmpty) {
      _showValidationDialog(
        'Validation Error',
        'Please add at least one item to save purchase inward.',
      );
      return;
    }
    ...
    try {
      ...
      final Map<String, dynamic> data2Map = {
        ...
        "supRefNo": _supRefNoCtrl.text,
        "date": "${docDtController.text} 00:00:00.000",
        "remark": _remarkCtrl.text,
        "grossAmount": calculatedGrossAmt.toStringAsFixed(2),
        "discount": calculatedDisc.toStringAsFixed(2),
        "taxAmount": taxAmt.toStringAsFixed(2),
        "otherCharges": otherChrgs.toStringAsFixed(2),
        "amount": calculatedAmount.toStringAsFixed(2),
        "roundOff": rdOff,
        "netAmount": calculatedNetAmt.toStringAsFixed(2),
        // Details from details page
        "transporter": _detailsData['transporter'] ?? '',
        "broker": _detailsData['broker'] ?? '',
        "commPercent": _detailsData['commPercent'] ?? '0',
        "lrNo": _detailsData['lrNo'] ?? '',
        "lrDate": _detailsData['lrDate'] ?? '',
        "currency": _detailsData['currency'] ?? 'INR',
        "eWayBillNo": _detailsData['eWayBillNo'] ?? '',
        "deliverySchedule": _detailsData['deliverySchedule'] ?? 'Common',
        "deliveryDays": _detailsData['deliveryDays'] ?? '0',
        "deliveryDate": _detailsData['deliveryDate'] ?? '',
        "discTerms": _detailsData['discTerms'] ?? '',
        "pytDays": _detailsData['pytDays'] ?? '0',
        "dueDate": _detailsData['dueDate'] ?? '',
        "partyAddress": _detailsData['partyAddress'] ?? '',
      };
      ...
      for (var item in addedItems) {
        ...
        if (sizes.isNotEmpty) {
          for (var size in sizes) {
            ...
            final double purRate = size['PurRate'] as double? ?? 0;
            
            if (sizeQty > 0) {
              itemsArray.add({
                "designcode": styleCode,
                "soDocId": item['soDocId'] ?? 0,
                "soDocDtlId": item['docDtlId'] ?? 0,
                "soDocDtlSzId": soDocDtlSzId,
                "stkId": 0,
                "mrp": mrp.toString(),
                "WSP": "0",
                "size": sizeName,
                "TotQty": sizeQty.toString(),
                "Note": "",
                "color": shadeName,
                "Qty": sizeQty.toString(),
                "PurRate": purRate.toString(),
                "cobrid": coBrId,
                "user": userId,
                "barcode": "",
              });
      ...
      print('Saving Purchase Inward - Payload: ${jsonEncode(payload)}');

      Map<String, dynamic> response;

      if (_isUpdateMode) {
        response = await ApiService.updatePurchaseInward(payload);
      } else {
        response = await ApiService.savePurchaseInward(payload);
      }
      ...
      if (response['status'] == 'success') {
        ...
        _showErrorSnackBar(response['message'] ?? 'Failed to save purchase inward');
      }
    } catch (e) {
      print('Error saving purchase inward: $e');
      _showErrorSnackBar('Error saving purchase inward: $e');
    } finally {
                  ...
                  Expanded(
                    child: RichText(
                      text: TextSpan(
                        ...
                        children: [
                          TextSpan(
                            text: _isUpdateMode ? 'Purchase Inward ' : 'Purchase Inward ',
                          ),
                          TextSpan(
                            text: docNo,
                            style: TextStyle(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Expanded(
                                child: _buildTextField('Sup Ref No', _supRefNoCtrl),
                              ),
                              const SizedBox(width: 12),
                              Expanded(
          ...
          children: [
            SizedBox(
              ...
              child: FloatingActionButton(
                heroTag: 'so',
                onPressed: isSupplierSelected ? _openSOSelection : null,
  ...
  void _deleteItem(int docDtlId) {
    setState(() {
      ...
      selectedSOItems.removeWhere((item) => item['docDtlId'] == docDtlId);
      _selectedDocDtlIds = selectedSOItems
          .map<int>((item) => item['docDtlId'] as int)
          .toList();
      _calculateTotals();
          ...
          children: [
            ...
            Expanded(
              child: SizedBox(
                height: 50,
                child: ElevatedButton.icon(
                  onPressed: (hasItems && !_isSaving) ? _saveInward : null,
                  icon: _isSaving
                      ? const SizedBox(
                          height: 20,
                          width: 20,
