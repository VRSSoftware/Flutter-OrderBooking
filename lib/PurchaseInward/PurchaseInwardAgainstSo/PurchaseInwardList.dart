...
class SOForPurchaseInwardListScreen extends StatefulWidget {
  final String supplierKey;
  final List<Map<String, dynamic>> existingSelectedItems;

  const SOForPurchaseInwardListScreen({
    Key? key,
  ...
  @override
  _SOForPurchaseInwardListScreenState createState() => _SOForPurchaseInwardListScreenState();
}

class _SOForPurchaseInwardListScreenState extends State<SOForPurchaseInwardListScreen> {
  List<dynamic> _soItems = [];
  ...
  Future<void> _fetchSOItems() async {
    setState(() => _isLoading = true);

    try {
      final response = await PurchaseService.fetchPOForPurchaseInwardAgainstSO(
        coBrId: UserSession.coBrId ?? '',
        supplierKey: widget.supplierKey,
      );

      print('SO Items Response: $response');
      ...
      if (response['status'] == 'success' && response['data'] != null) {
        ...
        print('Already selected: ${existingSelectedIds.length}');
        print('Available to add: ${pendingItems.length}');
        
        setState(() {
          _soItems = pendingItems;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response['message'] ?? 'Failed to fetch SO items'),
            backgroundColor: Colors.orange,
          ),
        );
      }
    } catch (e) {
      print('Error fetching SO items: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error fetching SO items: $e'),
  ...
  Future<void> _fetchSizeDetailsAndAdd() async {
    ...
    try {
      ...
      final response = await PurchaseService.fetchSizeDetailsForPurchaseInward(requestBody);
      ...
      if (response['status'] == 'success' && response['data'] != null) {
        ...
        for (var item in sizeDetailsData) {
          ...
          final originalItem = _soItems.firstWhere(
            (soItem) => soItem['DocDtl_Id'] == docDtlId,
            orElse: () => null,
          );
          
          if (originalItem == null) continue;
          
          double balQty = (originalItem['BalQty'] as num?)?.toDouble() ?? 0;
          ...
          double purRate = (originalItem['PurRate'] as num?)?.toDouble() ?? 0;
          double amount = totalQty * purRate;
          ...
          for (var size in sizes) {
            transformedSizes.add({
              ...
              'MRP': (size['MRP'] as num?)?.toDouble() ?? 0,
          ...
          newItems.add({
            'docDtlId': docDtlId,
            'SONo': originalItem['Doc_No'] ?? '',
            'SODate': originalItem['Doc_Dt'] ?? '',
            ...
            'BalQty': balQty,
            'PurRate': purRate,
            'Qty': totalQty > 0 ? totalQty : balQty,
            'Amount': amount,
  ...
  void _filterItems(String searchText) {
    if (searchText.isEmpty) {
      setState(() {
        _filteredItems = List.from(_soItems);
      });
      return;
    }
    ...
    setState(() {
      _filteredItems = _soItems.where((item) {
        final soNo = item['Doc_No']?.toString().toLowerCase() ?? '';
        if (soNo.contains(lowerSearchText)) return true;

        final itemName = item['Item_Name']?.toString().toLowerCase() ?? '';
        if (itemName.contains(lowerSearchText)) return true;

        final styleCode = item['Style_Code']?.toString().toLowerCase() ?? '';
    ...
    return AppBar(
      ...
      title: TextField(
        ...
        decoration: const InputDecoration(
          hintText: 'Search by SO No, Item, Style...',
              ...
              ? const Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 16),
                      Text(
                        'No sales orders found for inward',
  ...
  Widget _buildItemCard(dynamic item, bool isSelected, int docDtlId) {
    ...
    final String soNo = item['Doc_No']?.toString() ?? 'N/A';
    final String soDate = item['Doc_Dt']?.toString() ?? 'N/A';
          ...
          child: ExpansionTile(
            ...
            title: Column(
              ...
              children: [
                ...
                Text(
                  'Shade: $shadeName | Bal Qty: $balQty $unit',
            ...
            trailing: Container(
              ...
              child: Text(
                '₹${(balQty * purRate).toStringAsFixed(2)}',
                style: TextStyle(
                  fontSize: 12,
                  fontWeight: FontWeight.w600,
