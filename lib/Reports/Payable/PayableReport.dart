...
class _PayableReportState extends State<PayableReport> {
  ...
  Future<void> _saveAndViewPdf(Uint8List pdfBytes) async {
    try {
      ...
      final fileName = 'Payable_Report_${selectedCustomer?.name}_${DateTime.now().millisecondsSinceEpoch}.pdf';
...
class _PdfViewerScreenState extends State<PdfViewerScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Payable Report - ${widget.customerName}'),
        backgroundColor: AppColors.primaryColor,
        foregroundColor: Colors.white,
        actions: [
          IconButton(
  ...
  Future<void> _savePdfPermanently() async {
    try {
      ...
      final fileName = 'Payable_Report_${widget.customerName}_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.pdf';
