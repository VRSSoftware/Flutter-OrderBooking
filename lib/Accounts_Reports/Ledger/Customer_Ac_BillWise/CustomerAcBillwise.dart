...
class CustomerAcBillwisePage extends StatefulWidget {
  const CustomerAcBillwisePage({super.key});

  @override
  State<CustomerAcBillwisePage> createState() => _CustomerAcBillwisePageState();
}

class _CustomerAcBillwisePageState extends State<CustomerAcBillwisePage> {
  ...
  Future<void> fetchCustomerBillwiseLedgers() async {
    setState(() => _isLoadingLedgers = true);
    try {
      final fetchedLedgers = await AccountReportService.fetchCustomerBillwiseLedgers();
      if (mounted) {
        setState(() {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to load customer billwise ledgers: $e')),
        );
      }
    }
  }
  ...
  String _getSubtitle() {
    ...
    if (filters.isEmpty) {
      return 'All Customer Ledgers (Bill Wise)';
    }
  ...
  Future<void> _viewReport() async {
    ...
    try {
      ...
      final pdfBytes = await AccountReportService.generateCustomerBillwiseReport(
      ...
      if (mounted && pdfBytes != null) {
        final fileName = 'Customer_Billwise_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.pdf';
                  ...
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) => CommonFilterPage(
                        title: 'Customer Billwise Filters',
                        reportType: 'CustomerLedgerBillWise',
                        states: states,
                        cities: cities,
                        ledgers: ledgers,
