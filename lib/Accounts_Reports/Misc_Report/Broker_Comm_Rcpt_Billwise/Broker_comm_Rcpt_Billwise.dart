...
class BrokerCommissionReceiptPage extends StatefulWidget {
...
class _BrokerCommissionReceiptPageState
  ...
  int get _filterCount {
    int count = 0;
    if (selectedBrokers.isNotEmpty) count++;
  ...
  Future<void> fetchBrokers() async {
    ...
    try {
      final fetchedBrokers = await AccountReportService.fetchBrokers();
      if (mounted) {
        setState(() {
          brokers = fetchedBrokers;
  ...
  Future<void> _viewReport() async {
    ...
    try {
      ...
      final brokerKeys = selectedBrokers.map((e) => e.key).toList();
